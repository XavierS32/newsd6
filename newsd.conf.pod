...
=item ErrorLog <stderr | syslog | |program | pathname>

Specifies the file to use for logging information, errors, and
debugging information:

    stderr     -- messages are sent to standard error.
    syslog     -- messages are logged via syslog(3)
    |program   -- pipes error messages to a program
    pathname   -- appends to a specified filename (e.g. /var/log/newsd.log)

When "|program" is employed, typically a program like I<logger(3)> is used, e.g.

    ErrorLog |/usr/bin/logger --msgid newsd

It is strongly recommended that absolute paths be used with "program" to
avoid security issues with a possible insecure PATH environment variable.

The default is "stderr".

=item HostnameLookups <on|off>

When a client connects to the news server, this directive
controls whether the server looks up the client's IP address:

    off    -- disable IP address lookups (only IP addresses are logged)
    on     -- enable IP address lookups
    double -- enable IP address lookup and verification.
...
=item LogLevel <error|info|debug>

Specifies the type of information to be logged:

    error   -- only error messages are logged.
    info    -- informational and error messages are logged.
    debug   -- debugging, informational, and error messages are logged.

The default is "info".

=item MaxClients <value>

Specifies the maximum number of simultaneous clients. A <value> of 0
...
The default is 1000000 (one megabyte).

=item ServerName <name>

Specifies the hostname reported to NNTP clients.
The default is the name reported by I<hostname(1)>.

=item SendMail <command>

Specifies the command (and any options) to use when sending mail messages.

The default is "/usr/local/sbin/sendmail".
...
connection open; specify 0 to disable the idle timeout.

The default is 43200 seconds (12 hours).

=item User <username>

Specifies the user account the I<newsd> process will run as.
This user should exist in the /etc/passwd file, and is typically
assigned to a similarly named user group.

The default user account name is "news".
...
Use this if groups will be large and contain many thousands of articles.
Helps prevent OS having to walk large linear directories of articles
by breaking them up into subdirectories not more than 1000 articles each.

So when disabled with "no", articles are saved this way:

    /path/fltk/general/1
    /path/fltk/general/2
    /path/fltk/general/3
            :
    /path/fltk/general/99998
    /path/fltk/general/99999
    /path/fltk/general/100000
    /path/fltk/general/100001
            :
	   etc

..and when this feature is enabled, articles are saved this way:

    /path/fltk/general/0/1
    /path/fltk/general/0/2
    /path/fltk/general/0/3
            :
    /path/fltk/general/0/998
    /path/fltk/general/0/999
    /path/fltk/general/1000/1000
    /path/fltk/general/1000/1001
    /path/fltk/general/1000/1002
            :
    /path/fltk/general/1000/1998
    /path/fltk/general/1000/1999
    /path/fltk/general/2000/2000
    /path/fltk/general/2000/2001
            :
	   etc

If enabled, then groups should not have entirely numeric names (e.g. "alt.1000"),
to prevent newsd from confusing message dirs with group names.

=item PostCommand <- | program>

Sets an optional program (or script) to be run immediately after a
new newsgroup article has been saved into the spool directory with
an article number assigned to it.

The command can also contain space separated arguments, such as to invoke a script
with a named interpreter, e.g.

    PostCommand /usr/bin/python3 /path/to/myscript.py
                ---------------- --------------------
		Interpreter      Script path

At the time the script runs, three arguments are appended to the command in
the following order: <group> <article#> <msgpath>, where:

    <group>      -- full groupname, e.g. "fltk.general"
    <article#>   -- the unique article#, e.g. "12037"
    <msgpath>    -- the absolute path to the article file, e.g. /some/path/spool/fltk/general/12037

Example: If the newsd.conf file contains:

    PostCommand /path/to/my_script
    SpoolDir    /var/spool/news
    MsgModDirs  yes

..and a new article #12037 for group "fltk.general" has just been posted
(either via NNTP, or added via the "newsd -mailgateway"), then the full
command invoked to post-process the article passed to I<system(3)> would be:

    /path/to/myscript fltk.general 12037 /var/spool/news/fltk/general/10000/12037
    ----------------- ------------ ----- ----------------------------------------
              |            |         |                    |           \____/
            <cmd>       <group>   <msgnum>             <msgpath>       MsgModDir

The "myscript" program can then use the arguments to post-process the article
if need be. For example, converting the article to HTML format, and expand
mime attachments.

WARNING: It is strongly advised that only absolute paths are used for
the PostCommand to avoid security issues with a possibly insecure PATH
environment variable.  The command is executed I<system(3)>, running as
the configured news user.

The default is "-", which disables the use of a post command.

=back

=head1 SPOOL DIRECTORY, ".config" AND ".info" FILES
...
=head2 GROUP ".config" FILES

Each group's ".config" file is in a simple plain text format
similar to newsd.conf file, and appears in the spool directory
for each group:
...
when people first subscribe to the newsgroup. Example:

    description rush general discussions.
...
Example:
    ...
    creator   manager@somedomain.com

=item postok 0|1

Controls whether articles can be posted to this group.

    0 -- indicates posting is not allowed (read-only group)
    1 -- indicates anyone can post to this group.
...
"0" to disable the limit. The default limit is "1000". Example:
